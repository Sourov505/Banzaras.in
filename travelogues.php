...
function showImage()
{
  ...
  {
    $SlideArray= array('First','Second','Third', 'Fourth','Fifth');
...
function showDetail()
{
  if(isset($_GET['destinationId']))
  {
    ...
    echo"</div></div><br>";
    if(!empty($detail[3]))
    {  echo"<button class=\"btn btn-success active\"><span class=\"glyphicon glyphicon glyphicon-hand-right\"></span> <a href=\"travelogues.php?blogId={$detail[3][0]}\"> Read Our Story</a> </button>";
    }
    else {
...
function showBlogs()
{
  $query="select blogId,title,images from blog where isActive=1 and isVisible=1";
  $allDetail= mysql_query($query) or die("Error in Query");
  if(!empty($allDetail))
    echo"<h2>Travelogues</h2>";
  echo "<div class=\"blog-thumbnail\"><ul>";
  while($result=mysql_fetch_array($allDetail))
  {
  $image=$result['images'];
  $title=$result['title'];
  ...
  $imageLocation=mysql_fetch_array($imageResult);
   $placeName=trim($title);
  echo"<li> <img src={$imageLocation['path']} class=\"w3-hover-grayscale\" alt={title}>";
  ...
  {
    echo "<span style='left:6px;'>$placeName</span>";
  }
  else
  {
  ...
  $Url=getUrl($blogId);
  echo"<button type=\"button\" class=\"btn btn-default\"> <a href={$Url}>Explore</a>
   </button></li>";
...
{
  $queryBlogs="select * from permalink where BlogId=$blogId";
...
function pagination()
  ...
  {
     $keyword="Blogs,Trek, Holiday";
     $description="A traveling Site for travel and living";
     $title='Travelogues';
